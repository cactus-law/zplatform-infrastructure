 ...
 */
@Service
public class AccEntryServiceImpl extends AbstractBasePageService<AccEntryQuery,AccEntry> implements AccEntryService{
    ...
    public void accEntryProcess(TradeInfo tradeInfo) throws AccBussinessException, AbstractBusiAcctException, NumberFormatException, AccBussinessException {
        ...
        List<PojoAccEntry> checkTxnNo  = accEntryDAO.getByTxnNo(tradeInfo.getTxnseqno(), tradeInfo.getBusiCode());
        ...
        // 根据交易流水查找分录规则
        List<PojoSubjectRuleConfigure> list = subjectRuleConfigureDAO.getRulesByBusiCode(tradeInfo.getBusiCode());
        ...
        if (tradeInfo.isSplit()) {
             // 有分账的情况
             txnsSplitAccountList = txnsSplitAccountDAO.getByTxnseqno(tradeInfo.getTxnseqno());
             if (txnsSplitAccountList == null || txnsSplitAccountList.size()==0) {
        ...
        for (PojoSubjectRuleConfigure rule : list) {
            if (log.isDebugEnabled()) {
                log.debug("分录规则"+rulesNo+++"：" + JSONObject.fromObject(rule));
            }
            // 取得详细科目
            ...
            if (CONTINUE.equals(subject))
                continue;
            // 分录科目标识
            String firstChar = String.valueOf(rule.getFlag().charAt(0)).toUpperCase();
            // 判断是否是收款方分账
            if (tradeInfo.isSplit() && "S".equals(firstChar)) {
                // 复制交易信息
                TradeInfo copyTradeInfo = BeanCopyUtil.copyBean(TradeInfo.class, tradeInfo);
	...
	public void accountBatch(List<Long> accEntry) throws AccBussinessException {
       for (Long id : accEntry) {
           ...
           if (account == null) {
               throw new AccBussinessException("E000014",new Object[]{entry.getAcctCode()});
           }
           ...
           // 得到记账金额
           Money updateAmount = Money.valueOf(getUpdateAmount(entry.getAmount().getAmount(),entry,account));
    ...
    /**
     * 分录金额计算
     * @param ordform 分录金额计算规则 例：（A+B）
     * @param tradeInfo 交易流水
     * @return
     * @throws AccBussinessException 
     */
    private BigDecimal getAccEntryAmount(String ordform, TradeInfo tradeInfo) throws AccBussinessException {
     ...
     * @throws AccBussinessException 
     */
    private String getSubjectByFlag(String subject, TradeInfo tradeInfo) throws AbstractBusiAcctException, NumberFormatException, AccBussinessException {
        // 如果是具体科目，则原样返回。
        ...
        if ("S".equals(firstChar)) {
            ...
            // 根据通道找到相应的成本账户
            PojoChannel channel = channelDAO.getByChnlCode(tradeInfo.getChannelId());
            if (channel == null) {
                log.error("根据通道ID没找到相应的通道 ->通道ID："+tradeInfo.getChannelId());
                throw new AccBussinessException("E000020", new Object[]{tradeInfo.getChannelId()});
            }
        ...
        String account = busiAcctService.getAccount(Usage.fromValue(usage), memberId);
        return account;
    }
    /**
     * 得到记账所需要更新的金额
     * @如果分录规则的CRDR和详细科目的CRDR一致的话，则余额为增加
     ...
     * @return
     */
    private BigDecimal getUpdateAmount(BigDecimal amount, PojoAccEntry entry, PojoAccount account) {
        return entry.getCrdr().equals(account.getCrdr()) ? amount : amount.negate();
    }
    /**
     * 记录分录流水
     * @param tradeInfo
     ...
     * @throws NumberFormatException
     */
    private BigDecimal  insertEntry(TradeInfo tradeInfo, PojoSubjectRuleConfigure rule, BigDecimal balanceTest) throws AccBussinessException, AbstractBusiAcctException, NumberFormatException {
        // 取科目
        String subject = getSubjectByFlag(rule.getFlag(), tradeInfo);
        // 分录金额计算
        BigDecimal amount = getAccEntryAmount(rule.getEntryAlgorithm(), tradeInfo);
        // 保存分录流水
        PojoAccEntry entry = new PojoAccEntry();
        entry.setAcctCode(subject);// 账号/科目号
        entry.setCrdr(rule.getCrdr());// 借贷标志 1-借 2-贷
        entry.setAmount(Money.valueOf(amount));// 金额
        entry.setTxnseqno(tradeInfo.getTxnseqno());// 交易流水号
        ...
        if ("1".equals(rule.getSyncFlag())) {
            ...
            // 得到记账金额
            Money updateAmount = Money.valueOf(getUpdateAmount(amount,
                    entry, account));

            // 金额增加时，判断是否止入或冻结
            if (updateAmount.compareTo(Money.valueOf(BigDecimal.ZERO)) >= 0
            ...
            // 余额是否足够
            BigDecimal calcAmount =account.getBalance().plus(updateAmount).getAmount(); 
            ...
            entry.setStatus(AccEntryStatus.NOT_ACCOUNTED);// 未记账
        }
    ...
    protected List<AccEntry> getItem(int offset,
      ...
      List<PojoAccEntry> list=  accEntryDAO.getListByQuery(offset, pageSize, example);
